...
export default function Screen5Page() {
  ...
  // ---- Translation via Docker Qwen -----------------------------------------
  const selectLanguage = async (lang: string) => {
    setLanguage(lang);
    if (lang === "English") return;
    if (translations[lang]) return; // cached
    const original = transcript?.segments || [];
    if (original.length === 0) return;

    setTranslatingLang(lang);
    setStatus(`Translating to ${lang} with Docker Qwen…`);
    try {
      const res = await fetch("/api/translate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ segments: original, targetLanguage: lang }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setTranslations((prev) => ({ ...prev, [lang]: data.segments }));
        setStatus(`Showing ${lang} translation · Qwen ${data.model}`);
      } else {
        setStatus(`Translation error: ${data.error || "failed"}`);
        setLanguage("English");
      }
    } catch (err: any) {
